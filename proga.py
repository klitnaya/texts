...
def case1():
    ...
    url = 'https://github.com/klitnaya/texts/blob/main/text1.txt'
    url2 = 'https://github.com/klitnaya/texts/blob/main/text2.txt'
    response = urllib.request.urlopen(url)
...
def case2():
    ...
    for i in range(CNT_RANDOM_TEXTS):
        ...
        with open('./random_/case2_text_{0}'.format(i + 1), 'w') as f:
            f.write(text2)
...
def case3():
    ...
    response = urllib.request.urlopen(url)
    text1 = response.read().decode()
    url2 = 'https://github.com/klitnaya/texts/blob/main/random_text.txt'
    response = urllib.request.urlopen(url2)
    text2 = response.read().decode()
    min_len = min(len(text1), len(text2))
    text1 = text1[:min_len]
...
def case4():
    ...
    for i in range(CNT_RANDOM_TEXTS):
        ...
        with open('./random_/random_less_{0}'.format(i + 1), 'w') as f:
            f.write(text1)
        text2 = gen_random_letters(LEN_RANDOM_TEXT)
        with open('./random_/random_less2_{0}'.format(i + 1), 'w') as f:
            f.write(text2)
        s += match_perc(text1, text2)
    s /= CNT_RANDOM_TEXTS
...
def case5():
    ...
    url = 'https://github.com/klitnaya/texts/blob/main/random_less.txt'#'http://www.gutenberg.org/files/4300/4300-0.txt'
    url2 = 'https://github.com/klitnaya/texts/blob/main/random_less2.txt'
    response = urllib.request.urlopen(url)
    text1 = response.read().decode()
    response = urllib.request.urlopen(url2)
    text2 = response.read().decode()
